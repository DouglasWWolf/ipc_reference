...
wait_for_connection:
    ...
    // Fetch the name of the proxy server, if any
    const char* proxy = config.connect_ip.empty() ? nullptr : config.connect_ip.c_str();

    if (proxy)
    {
        // If we're in verbose mode, tell the user who we're connecting to
        if (m_verbose) printf("Connecting to %s on port... %i\n", proxy, config.connect_port);

        // If we can't connect to the proxy, complain
        if (!m_socket.connect(config.connect_ip, config.connect_port))
        {
            printf("Connection to %s on port %i failed!\n", proxy, config.connect_port);
            exit(1);
        }

        // In verbose mode, tell the user that we sucessfully connected to the proxy
        if (m_verbose) printf("Connected!\n");
    ...
    while (m_socket.getline(buffer, sizeof buffer))
    {
        ...
        // Save the raw buffer in face the command handler needs it
        m_raw_line = buffer;
    ...
    // Release any resources we have associated with this socket
    m_socket.close();
